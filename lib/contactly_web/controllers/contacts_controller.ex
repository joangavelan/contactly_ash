defmodule ContactlyWeb.ContactsController do
  ...
  def import_csv(conn, %{"file" => %Plug.Upload{path: path, content_type: "text/csv"}}) do
    ...
    case Contacts.upload_csv_contacts(path, current_user) do
  ...
  def export_csv(conn, _params) do
    ...
    with {:ok, contacts} <- Contacts.list_contacts(actor: current_user),
         {:ok, csv} <- Contacts.generate_csv_contacts(contacts) do
      conn
